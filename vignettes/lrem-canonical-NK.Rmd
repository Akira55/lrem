...
names(result) <- c("ds", "ss", "output", "inf", "ni")
result["t"] <- 0:(steps - 1)
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = ds)) +  ylab("Demand Shock")
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = ss)) + ylab("Supply Shock")
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = output))  + ylab("Output")
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = inf))  + ylab("Nominal Interest Rate")
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = ni)) + ylab("Inflation")
```
