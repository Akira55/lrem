...
**NOTE**: we will define $\tilde{E}$, $\tilde{A}$ as `A` and `B` in `lrem` package.
...
policy <- lre_auto(A, E, B, Phi, ny = 5)

steps <- 100
out <- impulse(policy$g, policy$h, x0 =c(0,0,0,0,0) , t = steps, e1 = c(1, 0))
```

```{r}
#plotting result
result <- data.frame(out)
names(result) <- c("demand shock", "supply schock", "output", "inflation", "nominal interest rate")
result["t"] <- 0:(steps - 1)
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = demandShock))
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = supplyShock))
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = output))
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = inflation))
```

```{r}
ggplot(result) + geom_line(aes(x = t, y = nominalInterestRate))
```
